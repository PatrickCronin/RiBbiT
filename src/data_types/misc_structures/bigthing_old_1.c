...
int bigthing_prepare_stcs(bigthing b)
{
    ...
    b->stcs = (ulvlist *) check_malloc (sizeof(ulvlist) * NUM_CLASSES);
    for (i=0; i < NUM_CLASSES; i++) {
	b->stcs[i] = ulvlist_constructor(stc_free_shallow, NULL, NULL, (void*) stc_shallow_copy, NULL);
    }
	...
	do {
		...
		ulvlist_insert_end(b->stcs[stc_get_class(s)], s);
...
void bigthing_create_necessary_free_periods(bigthing b, int next_stc_id)
{
    ...
    */
	...
	num_periods = 0;
	ulvlist_init_pos(b->stcs[class_id]);
	while ((temp_stc = ulvlist_data_at_pos(b->stcs[class_id]))  != NULL) {
	    num_periods = num_periods + stc_get_num_periods(temp_stc);
	    ulvlist_move_pos(b->stcs[class_id]);
	}
	...
	for ( ; num_periods < (NUM_DAYS * NUM_PERIODS); num_periods++) {
	    ...
	    ulvlist_insert_end(b->stcs[class_id], temp_stc);
	    next_stc_id++;
	}
    }
}
